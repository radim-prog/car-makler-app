...
export default function SupplierOnboardingDocumentsPage() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch("/api/auth/supplier-onboarding", {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          step: 2,
          data: {
            documents: [businessDoc.url, idDoc.url],
          },
        }),
      });

      if (res.ok) {
        router.push("/parts/onboarding/approval");
      } else {
      ...
      <div className="max-w-lg mx-auto px-4 py-6 space-y-5">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-1">
            Nahrajte potřebné dokumenty pro ověření
          </p>
        </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-200">
          ...
          {businessDoc ? (
            <div className="flex items-center gap-2 bg-green-50 rounded-lg p-3">
              ...
              <span className="text-sm text-green-700 truncate">{businessDoc.name}</span>
            </div>
          ) : (
            <Button
              variant="outline"
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (file) handleUpload(file, setBusinessDoc);
            }}
          />
        </div>
        ...
        <div className="bg-white rounded-xl p-4 border border-gray-200">
          ...
          <p className="text-xs text-gray-500 mb-3">
            Fotografie nebo sken občanského průkazu
          </p>
          {idDoc ? (
            <div className="flex items-center gap-2 bg-green-50 rounded-lg p-3">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-5 h-5 text-green-500 shrink-0">
              ...
              <span className="text-sm text-green-700 truncate">{idDoc.name}</span>
            </div>
          ) : (
            <Button
              variant="outline"
          ...
          <input
            ...
            onChange={(e) => {
              const file = e.target.files?.[0];
              if (file) handleUpload(file, setIdDoc);
            }}
          />
        </div>
