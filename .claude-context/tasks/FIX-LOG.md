...
| FIX-008 | 2026-04-14 | implementátor | Prisma client regen | `package.json:16` | Přidán `"postinstall": "prisma generate"` do scripts. Po `npm install` / `npm ci` (např. při změně `prisma/schema.prisma` nebo dependencies) se Prisma klient automaticky regeneruje — bez ručního `npx prisma generate`. Pozn.: `prisma.config.ts` validuje DATABASE_URL, takže postinstall vyžaduje `.env` (sandbox/prod ho mají; dev potřebuje `.env.local`). | `2ab332b` | **F-016** (P1, stale Prisma client riziko při deployi) | ⏳ čeká na deploy + kontrolor smoke | ⏳ |
...
| 2026-04-14 | implementátor | FIX-008 (postinstall prisma generate, commit `2ab332b`, F-016) — čeká deploy + kontrolor smoke. |
